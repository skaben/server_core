...
class TestPrivateAlertStateApi(APITestCase):
    ...
    def test_retrieve_alert_states(self):
        for x in range(2):
            AlertState.objects.create(name=f"test_state_{x}", info="notimportant", current=False)
        res = self.client.get(API_URL)
    ...
    def test_alert_state_set_current_solitude(self):
        ...
        """
        state = AlertState.objects.create(name="statename", info="testinfo", current=False)
    ...
    def test_alert_state_set_current_normal(self):
        ...
        old = AlertState.objects.create(name="stateold", info="test2", current=True)
        new = AlertState.objects.create(name="statenew", info="test", current=False)
        counter = AlertCounter.objects.create(value="100500", comment="test")
    ...
    def test_create_alert_state_fail(self):
        ...
        name = "notastate"
        payload = {"name": name, "current": True}
