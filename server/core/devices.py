...
FULL_DEVICE_LIST = [
    (*TOPIC.get('rgb'), None, None), (*TOPIC.get('pwr'), None, None),
    (*TOPIC.get('scl'), None, None), (*TOPIC.get('box'), None, None),
...
class DeviceConfig:
    ...
    def topics(self, _type: str | None = None) -> List[str]:
        if not _type:
            ...
            return [
                device.topic for device in self.devices if device.type == _type
            ]
